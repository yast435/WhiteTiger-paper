...
WhiteTiger contains 6193 tasks organized into eight scenario categories: Kitchen and Dining (1304 tasks), Home Living (1136), Office and Education (972), General Manipulation (838), Retail and Healthcare (765), Industrial Manufacturing (577), Agriculture and Fresh Produce (475), and Warehousing, Packaging, and Logistics (126). Figure~\ref{fig:scenario-task-coverage} reports the task distribution across these scenario categories.

Scenario coverage determines how policy learning encounters variation in environment layout, object affordance, task objective, and interaction context. WhiteTiger contains 6193 tasks spanning short-horizon primitives and longer task chains. Many tasks compose lower-level manipulation skills such as grasping, placing, pushing, pulling, handing over, inserting, pressing, opening, closing, sorting, and arranging. This compositional structure supports direct action prediction, skill abstraction, temporal segmentation, and analysis of how reusable sensorimotor routines transfer across physical contexts.
...
% TODO: Regenerate scenario_task_coverage3.png and task_duration_distribution2.png using the updated 6193-task dataset inventory.
\begin{figure}[t]
\centering
...
\end{subfigure}
\caption{Task coverage and temporal diversity of WhiteTiger. The left panel reports task distribution across eight scenario categories, and the right panel reports task-level mean trajectory duration.}
...
\midrule
ARX-Loong & 11 & 1322 & 3300789 & 30.56 \\
...
AGIBOT G1 & 1739 & 154021 & 485095219 & 4491.62 \\
Fourier GR-2 & 1303 & 155590 & 173329898 & 1604.91 \\
Leju KUAVO & 413 & 46167 & 76055616 & 704.22 \\
...
\textbf{Total} & \textbf{6193} & \textbf{737455} & \textbf{1390532587} & \textbf{12875.3} \\
\bottomrule
\end{tabular}
\end{table}
...
The 14 platforms differ in morphology, end-effector design, visual sensing, and recorded state/action dimensionality. Table~\ref{tab:robot-config} summarizes robot category, arm degrees of freedom, end-effector type, platform-specific state/action dimension, additional recorded body components, and camera layout. Across WhiteTiger, these schemas range from compact 14D dual-arm-and-gripper representations to 41D whole-body representations. All trajectories are recorded at 30~Hz.

The modality schemas expose substantial embodiment heterogeneity beyond platform identity alone. ARX-Loong, UR5e, and Galaxea R1 use 14D arm-and-gripper schemas; Astribot S1 records a 25D schema that additionally includes neck, waist, and mobile-base components; Franka records a 30D schema containing dual-arm joints, grippers, end-effector positions, and orientations; AGIBOT A2 records a 28D schema with dexterous hands and head joints; and Fourier GR-2 reaches 41D by additionally representing head, legs, and waist. QingLong appears in two collection configurations: a 16D arm-and-gripper schema and a 33D whole-body schema that additionally includes neck, waist, and leg components. WhiteTiger retains these configuration-dependent native schemas rather than projecting them into an artificially shared control space.
