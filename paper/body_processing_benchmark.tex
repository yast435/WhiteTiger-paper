...
WhiteTiger converts heterogeneous human-teleoperated robot operation records into a consistent dataset for large-scale policy training. As summarized in Fig.~\ref{fig:data-processing-pipeline}, the pipeline transforms platform-specific HDF5 trajectories into a model-ready LeRobot v2.1 dataset through validation, filtering, metadata standardization, embodiment-schema alignment, and format conversion. The processing pipeline emphasizes three requirements: preserving embodiment-specific control information, filtering invalid or task-inconsistent trajectories, and producing a unified representation that can be loaded by modern robot learning systems.
...
The benchmark results show that one epoch of full-parameter fine-tuning on WhiteTiger substantially reduces normalized offline open-loop prediction error across the 13 evaluated platforms and 42 external paired task-dataset records. At the aggregate level, the WhiteTiger-trained model reduces Normalized ALL MSE from 0.157283 to 0.005051, corresponding to a \textbf{96.79\%} relative improvement. It also reduces Normalized Joint MSE from 0.116746 to 0.000680, corresponding to a \textbf{99.42\%} relative improvement. The per-platform comparisons are visualized in Fig.~\ref{fig:per-platform-zero-shot}.

This indicates that WhiteTiger provides effective supervision for improving full-vector and joint-related action prediction on evaluation data that were not included in the WhiteTiger training corpus.

\subsection{Per-platform results}

Because WhiteTiger is a multi-embodiment dataset, aggregate metrics alone are insufficient. We therefore also analyze the results separately for each evaluated platform. Across all 13 platforms, the WhiteTiger-trained model achieves lower Normalized ALL MSE and Normalized Joint MSE than the no-training baseline.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/per_platform_zero_shot_dumbbell.jpg}
\caption{Per-platform offline zero-shot evaluation on datasets external to WhiteTiger. Each row compares the no-training GR00T N1.6 baseline with the WhiteTiger-trained model after one epoch of full fine-tuning on WhiteTiger for Normalized ALL MSE and Normalized Joint MSE. Connected markers show the change after WhiteTiger fine-tuning; lower is better. Each platform value is the unweighted mean over its paired task-dataset records. QingLong contains six records, while each of the remaining platforms contains three.}
...
Task-level results further show that WhiteTiger fine-tuning improves normalized offline prediction across the 42 external paired task-dataset records. Task-level reporting is especially useful because different platforms may have different action spaces, control frequencies, and end-effector structures.
...
The strongest improvements are observed on records where the no-training checkpoint has relatively large action prediction error. The remaining residual errors after WhiteTiger fine-tuning are small in absolute value, but they can still vary across platforms and tasks due to differences in action representation and task dynamics.

\subsection{Planned real-world evaluation of event-guided value learning}

The offline zero-shot benchmark evaluates the first design goal of WhiteTiger: large-scale dense trajectories for foundation-model adaptation and cross-dataset action prediction. To evaluate the second design goal, event- and keyframe-level temporal annotations for goal-oriented learning, we reserve a real-world evaluation protocol comparing policies trained with and without event-derived value supervision.

In the planned evaluation, both policies will start from the same supervised fine-tuned model and will be evaluated on the same real-robot tasks under randomized initial configurations. The baseline policy will use dense trajectory supervision without event-derived value learning, while the event-guided policy will use WhiteTiger event/keyframe annotations to construct task-progress supervision based on subgoal and event completion. This setting is designed to isolate the effect of WhiteTiger's temporal annotations on closed-loop execution.

The planned evaluation will cover one to three robot embodiments and one to three representative manipulation tasks. We will report execution success rate as the primary metric, and additionally analyze event completion rate, failure stage distribution, and time to completion. This experiment will quantify whether WhiteTiger's temporal annotations can improve real-world execution by providing explicit supervision over task progress. Until these experiments are completed, the current paper treats event-guided value learning as a reserved evaluation direction rather than a demonstrated result.

\subsection{Additional benchmark extensions}

The current benchmark provides a dataset-level held-out offline zero-shot comparison. Future versions can extend the benchmark with data-scaling evaluation, held-out platform evaluation, closed-loop simulation evaluation, and completed real-world rollout success-rate experiments for event-guided learning.
